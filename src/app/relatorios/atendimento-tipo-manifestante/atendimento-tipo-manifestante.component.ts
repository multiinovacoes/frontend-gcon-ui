...
@Component({
  selector: 'app-atendimento-tipo-manifestante',
  templateUrl: './atendimento-tipo-manifestante.component.html',
  styleUrls: ['./atendimento-tipo-manifestante.component.css']
})
export class AtendimentoTipoManifestanteComponent implements OnInit {
 ...
 ngOnInit(): void {
  ...
  pesquisa() {
    if (this.filtro.dataFinal.getTime() < this.filtro.dataInicial.getTime()){
      ...
      this.relatorioService.pesquisarTipoManifestante(this.filtro)
     .then(data =>{
       if (data.length == 0){
...
calculateTotal() {
 ...
 qtdAtendimentos() {
  ...
  pesquisarTipoManifestante(this.filtro).
      then(resultado => {
