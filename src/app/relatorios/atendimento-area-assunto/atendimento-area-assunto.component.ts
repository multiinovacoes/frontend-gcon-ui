...
@Component({
  selector: 'app-atendimento-area-assunto',
  templateUrl: './atendimento-area-assunto.component.html',
  styleUrls: ['./atendimento-area-assunto.component.css']
})
export class AtendimentoAreaAssuntoComponent implements OnInit {
 ...
 ngOnInit(): void {
  ...
  pesquisa() {
    if (this.filtro.dataFinal.getTime() < this.filtro.dataInicial.getTime()){
      ...
      this.relatorioService.pesquisarAreaAssunto(this.filtro)
     .then(data =>{
       if (data.length == 0){
...
calculateTotal() {
 ...
 qtdAtendimentos() {
  ...
  pesquisarAreaAssunto(this.filtro).
      then(resultado => {
