...
@Component({
  selector: 'app-atendimento-origem',
  templateUrl: './atendimento-origem.component.html',
  styleUrls: ['./atendimento-origem.component.css']
})
export class AtendimentoOrigemComponent implements OnInit {
 ...
 ngOnInit(): void {
  ...
  pesquisa() {
    if (this.filtro.dataFinal.getTime() < this.filtro.dataInicial.getTime()){
      ...
      this.relatorioService.pesquisarOrigem(this.filtro)
     .then(data =>{
       if (data.length == 0){
